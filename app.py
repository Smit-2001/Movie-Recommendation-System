...
def get_recommendations(title):
    ...
    return_df['Director'] = sorted(df2['director'].iloc[movie_indices])
    return_df['Movie Type'] = sorted(df2['genres'].iloc[movie_indices])
...
#        check = difflib.get_close_matches(m_name,all_titles,cutout=0.50,n=1)
        if m_name not in all_titles:
            ...
            for i in range(len(result_final)):
                ...
                Movie_Type.append(result_final.iloc[i][4])
                vote_average.append(result_final.iloc[i][5])
                if(len(str(result_final.iloc[i][1]))>3):
                    homepage.append(result_final.iloc[i][1])
                else:
                    homepage.append("#")
                
            return flask.render_template('found.html',movie_names=names,movie_homepage=homepage,search_name=m_name, movie_releaseDate=releaseDate,movie_director=director,movie_Type= Movie_Type,movie_rating=vote_average)
